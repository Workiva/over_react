...
// ignore_for_file: deprecated_member_use_from_same_package, prefer_function_declarations_over_variables, avoid_types_on_closure_parameters
...
import 'package:over_react/over_react.dart'
    show Dom, DummyComponent, ValidationUtil;
...
main() {
  void _commonNonInvokedBuilderTests(UiProps builder) {
    ...
    test('when a list with a non-invoked builder child passed in', () {
      expect(() => builder([Dom.div(), Dom.p()(), Dom.div()]),
          throwsArgumentError);
      verifyValidationWarning(contains(
          'It looks like you are trying to use a non-invoked builder as a child.'));
    });
    ...
    test('when non-invoked builder children are passed in variadically', () {
      expect(
          () => builder(Dom.div(), Dom.p()(), Dom.div()), throwsArgumentError);
      verifyValidationWarning(contains(
          'It looks like you are trying to use a non-invoked builder as a child.'));
    });
  }
  ...
  group('component base:', () {
    group('UiProps', () {
      ...
      group('renders a DOM component with the correct children when', () {
        ...
        test('a single child is passed in', () {
          ...
          var children = List<Text>.from(renderedNode.childNodes
        ...
        test('children are set via a list', () {
          ...
          var childNodes = renderedNode.childNodes
              .where((node) => node.nodeType != Node.COMMENT_NODE)
              .toList();
        ...
        test('children are set variadically', () {
          ...
          var renderedNode =
              renderAndGetDom(Dom.div()(firstChild, secondChild));
          var children = renderedNode.childNodes
              .where((node) => node.nodeType != Node.COMMENT_NODE)
              .toList();
      ...
      group('renders a composite Dart component with the correct children when',
        ...
        test('no children are passed in', () {
          ...
          expect(getDartChildren(renderedInstance), const TypeMatcher<List>(),
              reason: 'Should be a list because lists will be JSified');
        ...
        test('children are set via a list', () {
          ...
          expect(getJsChildren(renderedInstance), const TypeMatcher<List>(),
              reason: 'Should be a list because lists will be JSified');
        ...
        test('children are set variadically', () {
          ...
          var renderedInstance =
              render(TestComponent()(firstChild, secondChild));

          expect(getDartChildren(renderedInstance), const TypeMatcher<List>(),
              reason: 'Should be a list because lists will be JSified');
          expect(getDartChildren(renderedInstance),
              equals([firstChild, secondChild]));
      ...
      group('provides Map functionality:', () {
        test('is a Map', () {
          expect(TestComponentProps(), const TypeMatcher<Map>());
        });

        test(
            'toString() returns a user-friendly String that includes the key-value pairs',
            () {
      ...
      group('modifyProps()', () {
        test('passes the provided modifier itself', () {
          ...
          expect(
              props,
              equals(
                  {'className': 'original-class-name', 'id': 'original-id'}));
          ...
          expect(
              props,
              equals(
                  {'className': 'modified-class-name', 'id': 'original-id'}));
        });
      ...
      group('testId', () {
        test(
            'sets the correct value for the `data-test-id` key when setting the testId',
            () {
          var props = TestComponentProps();
        ...
        test(
            'does not set the value for the `data-test-id` when inTesting is false',
            () {
        ...
        test(
            'sets the correct value for the `data-test-id` key when adding a testId',
            () {
          var props = TestComponentProps();
          props.addTestId('value');
        ...
        test(
            'sets the correct value for the `data-test-id` key when adding multiple testIds through multiple calls to `addTestId`',
            () {
          var props = TestComponentProps();
        ...
        test(
            'does not set a value for the `data-test-id` when adding a testId but inTesting is false',
            () {
          UiProps.testMode = false;
        ...
        test(
            'does not set a value for the `data-test-id` when adding a `null` testId',
            () {
          var props = TestComponentProps();
    ...
    group('UiState', () {
      group('provides Map functionality:', () {
        test('is a Map', () {
          expect(TestStatefulComponentState(), const TypeMatcher<Map>());
        });
    ...
    group('UiComponent', () {
      ...
      group('copyUnconsumedProps()', () {
        test(
            'copies props, omitting keys from `consumedProps`, as well as reserved react props',
            () {
      ...
      group('copyUnconsumedDomProps()', () {
        test(
            'copies props, omitting keys from `consumedPropKeys`, as well as reserved react props',
            () {
          component = TestComponentComponent(testConsumedProps: [
            const ConsumedProps([], ['consumed1', 'consumed2'])
          ]);
          ...
          expect(
              component.copyUnconsumedDomProps(),
              equals({
                'tabIndex': true,
                'className': true,
              }));
      ...
      test(
          'forwardingClassNameBuilder() returns a new ClassNameBuilder based on the component\'s props',
          () {
        component.props = {
          'className': 'class-1',
      ...
      group('on unmount', () {
        ...
        test('should await future before disposing', () async {
          ...
          component.manageDisposer(
              () async => streamController.add('disposalFuture'));
        ...
        test('should complete delayed Future with ObjectDisposedException',
            () async {
          expect(
              component.getManagedDelayedFuture(
                  shortDuration,
                  expectAsync0(() {},
                      count: 0,
                      reason: 'Did not expect callback to be invoked.')),
              throwsA(const TypeMatcher<ObjectDisposedException>()));

          await unmountAndDisposal();
        });

        test('should call managed disposer returned by getManagedDisposer',
            () async {
          var disposerCalled = false;
          var disposer =
              component.getManagedDisposer(() async => disposerCalled = true);
        ...
        test('should cancel periodic timer', () async {
          var timer = component.getManagedPeriodicTimer(
              shortDuration,
              expectAsync1((_) {},
                  count: 0, reason: 'Did not expect callback to be invoked.'));
        ...
        test('should cancel timer', () async {
          var timer = component.getManagedTimer(
              shortDuration,
              expectAsync1((_) {},
                  count: 0, reason: 'Did not expect callback to be invoked.'));
        ...
        test('should cancel stream subscription returned by listenToStream',
            () async {
          var streamController = StreamController<Null>.broadcast();
          // ignore: cancel_subscriptions
          var streamSubscription = component.listenToStream(
              streamController.stream,
              expectAsync1((_) {},
                  count: 0,
                  reason:
                      'Did not expect event after cancelling subscription'));
        ...
        test(
            'should dispose managed Disposable returned by manageAndReturnDisposable',
            () async {
          var disposable = Disposable();
          expect(component.manageAndReturnDisposable(disposable),
              same(disposable));
        ...
        test(
            'should complete uncompleted managed Completer with ObjectDisposedException',
            () async {
        ...
        test('should cancel managed StreamSubscription', () async {
          var streamController = StreamController<Null>.broadcast();
          // ignore: cancel_subscriptions
          var streamSubscription = streamController.stream.listen(expectAsync1(
              (_) {},
              count: 0,
      ...
      group('`state`', () {
        group('getter:', () {
          test('returns a UiState view into the component\'s state map', () {
            expect(statefulComponent.state,
                const TypeMatcher<TestStatefulComponentState>());

            expect(statefulComponent.state,
                isNot(same(statefulComponent.unwrappedState)));

            statefulComponent.unwrappedState['testKey'] = 'testValue';
            expect(
                statefulComponent.state, containsPair('testKey', 'testValue'));
          });
          ...
          test('warns against setting state directly', () {
            ...
            verifyValidationWarning(
                contains('Never mutate this.state directly'));
            stopRecordingValidationWarnings();
          });
        });
    ...
    group('registerComponent()', () {
      group('attaches metadata to the specified component class:', () {
        ...
        group('`isWrapper`:', () {
          test('true', () {
            var reactComponentFactory =
                registerComponent(dummyComponentFactory, isWrapper: true);
            var meta = getComponentTypeMeta(reactComponentFactory.type);

            expect(meta, isNot(same(const ComponentTypeMeta.none())),
                reason: 'should have stored a new meta instance');
            expect(meta.isWrapper, isTrue);
          });

          test('false', () {
            var reactComponentFactory =
                registerComponent(dummyComponentFactory, isWrapper: false);
            var meta = getComponentTypeMeta(reactComponentFactory.type);
        ...
        group('`parentType`:', () {
          test('not specified', () {
            var reactComponentFactory =
                registerComponent(dummyComponentFactory);
            var meta = getComponentTypeMeta(reactComponentFactory.type);
        ...
        test('displayName', () {
          ...
          expect(reactComponentFactory.type.displayName,
              equals('testDisplayName'));
        });
        ...
        group('registers a type alias for', () {
          test('`componentClass`', () {
            ...
            var reactComponentFactory = registerComponent(dummyComponentFactory,
                componentClass: typeAlias);

            expect(getComponentTypeFromAlias(TestRegisterComponentClassAlias),
                equals(reactComponentFactory.type));
          });

          test('`factory`', () {
            UiFactory factoryAlias = ([_]) => null;
            var reactComponentFactory = registerComponent(dummyComponentFactory,
                builderFactory: factoryAlias);

            expect(getComponentTypeFromAlias(factoryAlias),
                equals(reactComponentFactory.type));
          });
        });
      });
    ...
    test(
        'registerAbstractComponent registers a type alias for a componentClass',
        () {
      Type typeAlias = TestRegisterComponentClassAlias;
    ...
    test(
        'registerAbstractComponent registers a type alias for a componentClass and parentType',
        () {
      Type typeAlias = TestRegisterComponentClassAlias;
      var parentFactory = registerComponent(() => DummyComponent());
      var reactComponentFactory =
          registerAbstractComponent(typeAlias, parentType: parentFactory);
      var meta = getComponentTypeMeta(reactComponentFactory.type);
...
final _TestComponentComponentFactory =
    registerComponent(() => TestComponentComponent());

class TestComponentComponent extends UiComponent<TestComponentProps> {
  @override
  final List<ConsumedProps> consumedProps;

  TestComponentComponent({List<ConsumedProps> testConsumedProps})
  ...
  @override
  TestComponentProps typedPropsFactory(Map propsMap) =>
      TestComponentProps(propsMap);
...
UiFactory<TestStatefulComponentProps> TestStatefulComponent =
    ([Map props]) => TestStatefulComponentProps(props);

class TestStatefulComponentProps extends UiProps {
  @override
  final ReactComponentFactoryProxy componentFactory =
      _TestStatefulComponentComponentFactory;
  @override
  final Map props;
...
final _TestStatefulComponentComponentFactory =
    registerComponent(() => TestStatefulComponentComponent());

class TestStatefulComponentComponent extends UiStatefulComponent<
    TestStatefulComponentProps, TestStatefulComponentState> {
  @override
  render() {}

  @override
  TestStatefulComponentProps typedPropsFactory(Map propsMap) =>
      TestStatefulComponentProps(propsMap);
  @override
  TestStatefulComponentState typedStateFactory(Map state) =>
      TestStatefulComponentState(state);
}

abstract class TestRegisterComponentClassAlias {}

class TestPropsMapViewMixin extends MapBase
...
class TestStateMapViewMixin extends MapBase
    with MapViewMixin, StateMapViewMixin
    implements Map {
  @override
  final Map state;
