  ...
  group('overReactBuilder', () {
    ...
    test('builds from basic component file', () async {
      await checkBuildForFile(
          'over_react|test/vm_tests/builder/test_fixtures/source_files/basic.dart',
          'over_react|test/vm_tests/builder/test_fixtures/source_files/basic.over_react.g.dart',
          '${p.absolute(p.current)}/test/vm_tests/builder/test_fixtures/gold_output_files/basic.over_react.g.dart.goldFile');
    });

    test('builds from basic multi-part library', () async {
      await checkBuildForFile(
          'over_react|test/vm_tests/builder/test_fixtures/source_files/basic_library.dart',
          'over_react|test/vm_tests/builder/test_fixtures/source_files/basic_library.over_react.g.dart',
          '${p.absolute(p.current)}/test/vm_tests/builder/test_fixtures/gold_output_files/basic_library.over_react.g.dart.goldFile');
    });

    test('does not produce a build output for a file with no over_react annotations', () async {
      var basicAsset = makeAssetId('over_react|test/vm_tests/builder/test_fixtures/source_files/no_annotations.dart');
    ...
    test('does not produce a build output for just a part file', () async {
      var basicAsset = makeAssetId('over_react|test/vm_tests/builder/test_fixtures/source_files/part_of_basic_library.dart');
      await runBuilder(builder, [basicAsset], reader, writerSpy, AnalyzerResolvers());
