      ...
      group('class:', () {
        ...
        group('more involved getters:', () {
          test('interfaces', () {
            ...
            ''').interfaces.map((i) => i.name.lexeme), ['Bar', 'Baz']);
          });
          ...
          test('mixins', () {
            ...
            ''').mixins.map((m) => m.name.lexeme), ['Bar', 'Baz']);
          });

          test('allSuperTypes', () {
            expect(parseAndGetSingleClassish('''
              class Foo extends Bar with Baz implements Qux {}
            ''').allSuperTypes.map((m) => m.name.lexeme), unorderedEquals(['Bar', 'Baz', 'Qux']));
          });
        });
      });
      ...
      group('mixin', () {
        ...
        group('more involved getters:', () {
          test('interfaces', () {
            ...
            ''').interfaces.map((i) => i.name.lexeme), unorderedEquals(['Bar', 'Baz']));
          });
          ...
          test('allSuperTypes', () {
            expect(parseAndGetSingleClassish('''
              mixin Foo on Bar implements Baz {}
            ''').allSuperTypes.map((m) => m.name.lexeme), unorderedEquals(['Bar', 'Baz']));
          });
        });
      });
    });
