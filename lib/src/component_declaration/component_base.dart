...
typedef TProps UiFactory<TProps extends UiProps>([Map backingProps]); // ignore: prefer_generic_function_type_aliases
...
typedef TProps BuilderOnlyUiFactory<TProps extends UiProps>(); // ignore: prefer_generic_function_type_aliases
...
abstract class UiComponent<TProps extends UiProps> extends react.Component with DisposableManagerProxy {
  ...
  TProps get props {
    ...
    if (typedProps == null) {
      typedProps = typedPropsFactory(inReactDevMode ? _WarnOnModify(unwrappedProps, isProps: true) : unwrappedProps);
...
abstract class UiStatefulComponent<TProps extends UiProps, TState extends UiState> extends UiComponent<TProps> {
  ...
  TState get state {
    ...
    if (typedState == null) {
    typedState = typedStateFactory(inReactDevMode ? _WarnOnModify(unwrappedState, isProps: false) : unwrappedState);
...
class _WarnOnModify<K, V> extends MapView<K, V> {
  ...
  _WarnOnModify(Map componentData, {@required this.isProps}): super(componentData);
...
typedef PropsModifier(Map props); // ignore: prefer_generic_function_type_aliases
