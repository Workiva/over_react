...
class ParsedDeclarations {
  ...
  factory ParsedDeclarations(CompilationUnit unit, SourceFile sourceFile, Logger logger) {
    ...
    List<CompilationUnitMember> propAncestorCompUnits;
    if (declarationMap[key_props].isNotEmpty) {
      // Only expect 1 instance of @Props() in a single file
      propAncestorCompUnits = getAncestorClassCompilationUnits(declarationMap[key_props]?.first, logger);
    }

    List<CompilationUnitMember> stateAncestorCompUnits;
    if (declarationMap[key_state].isNotEmpty) {
      // Only expect 1 instance of @State() in a single file
      stateAncestorCompUnits = getAncestorClassCompilationUnits(declarationMap[key_state]?.first, logger);
    }
    ...
    return new ParsedDeclarations._(
        ...
        propAncestorCompUnits: propAncestorCompUnits,
        stateAncestorCompUnits: stateAncestorCompUnits,
  ...
  ParsedDeclarations._({
      ...
      this.propAncestorCompUnits = propAncestorCompUnits,
      this.stateAncestorCompUnits = stateAncestorCompUnits,
...
//  }
  static PropsOrStateType getPropsOrStateType(CompilationUnitMember compUnit) {
    ...
    for (var annotation in compUnit.metadata) {
      ...
      if (propsClassType != PropsOrStateType.none) {
        if (name.compareTo(key_props) == 0 ||
            name.compareTo(key_abstractProps) == 0 ||
            name.compareTo(key_propsMixin) == 0) {
          return PropsOrStateType.props;
        }
        if (name.compareTo(key_state) == 0 ||
            name.compareTo(key_abstractState) == 0 ||
            name.compareTo(key_stateMixin) == 0) {
          return PropsOrStateType.state;
        }
      }
    }
    return PropsOrStateType.none;
  }
  ...
  final List<CompilationUnitMember> propAncestorCompUnits;
  final List<CompilationUnitMember> stateAncestorCompUnits;
