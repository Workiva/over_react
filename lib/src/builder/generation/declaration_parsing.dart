...
class ParsedDeclarations {
  factory ParsedDeclarations(
      CompilationUnit unit, SourceFile sourceFile, Logger logger) {
    ...
    void checkForMetaPresence(ClassDeclaration node) {
      ...
      final isStatic =
          (metaField?.isStatic ?? false) || (metaMethod?.isStatic ?? false);
      if (isNotNull) {
    ...
    void validateMetaField(ClassDeclaration cd, String expectedType) {
      ...
      final expectedInitializer =
          '${privateSourcePrefix}metaFor${cd.name.name}';

      final initializer =
          metaField.fields.variables.single.initializer?.toSource();
    ...
    unit.declarations.forEach((CompilationUnitMember _member) {
      _member.metadata.forEach((_annotation) {
        ...
        if (_member is! ClassDeclaration ||
            !(isPropsClass(annotation) || isStateClass(annotation))) {
          return;
        }
        ...
        if (!member.name.name.startsWith(privateSourcePrefix)) {
          error(
              'The class `${member.name.name}` does not start with `$privateSourcePrefix`. All Props, State, '
        ...
        final companionName =
            member.name.name.substring(privateSourcePrefix.length);
        final companionClass = unit.declarations.firstWhere(
            (innerMember) =>
                innerMember is ClassDeclaration &&
                innerMember.name.name == companionName,
            orElse: () => null);
        ...
        if (hasCompanionClass) {
          ...
          validateMetaField(companionClass,
              isPropsClass(annotation) ? 'PropsMeta' : 'StateMeta');
        } else {
    ...
    List<TopLevelVariableDeclaration> topLevelVarsOnly(
        String annotationName, Iterable<CompilationUnitMember> declarations) {
    ...
    List<ClassDeclaration> classesOnly(
        String annotationName, Iterable<CompilationUnitMember> declarations) {
      ...
      declarations.forEach((declaration) {
        if (declaration is ClassDeclaration) {
          ...
          error('`@$annotationName` can only be used on classes.',
              getSpan(sourceFile, declaration));
        }
      });
    ...
    declarationMap[key_factory] =
        topLevelVarsOnly(key_factory, declarationMap[key_factory]);
    ...
    [
      ...
      declarationMap[annotationName] = classesOnly(annotationName,
          declarationMap[annotationName] ?? const <CompilationUnitMember>[]);
    });
    ...
    Iterable<List<CompilationUnitMember>> requiredDecls =
        key_allComponentRequired
            .map((annotationName) => declarationMap[annotationName]);

    Iterable<List<CompilationUnitMember>> requiredDecls2 =
        key_allComponent2Required
            .map((annotationName) => declarationMap[annotationName]);

    Iterable<List<CompilationUnitMember>> optionalDecls =
        key_allComponentOptional
            .map((annotationName) => declarationMap[annotationName]);

    bool oneOfEachRequiredDecl2 =
        requiredDecls2.every((decls) => decls.length == 1);
    bool oneOfEachRequiredDecl =
        requiredDecls.every((decls) => decls.length == 1) ||
            oneOfEachRequiredDecl2;
    bool noneOfAnyRequiredDecl2 =
        requiredDecls2.every((decls) => decls.length == 0);
    bool noneOfAnyRequiredDecl =
        requiredDecls.every((decls) => decls.length == 0) &&
            noneOfAnyRequiredDecl2;

    bool atMostOneOfEachOptionalDecl =
        optionalDecls.every((decls) => decls.length <= 1);
    bool noneOfAnyOptionalDecl =
        optionalDecls.every((decls) => decls.length == 0);
    ...
    void _emitDuplicateDeclarationError(
        String annotationName, int instanceNumber) {
    ...
    if (declarationMap[key_component].isNotEmpty &&
        declarationMap[key_component2].isNotEmpty) {
      error(
    ...
    if (!areDeclarationsValid) {
      if (!noneOfAnyRequiredDecl) {
        if (declarationMap[key_component].isEmpty &&
            declarationMap[key_component2].isEmpty) {
    ...
    final factory = declarationMap[key_factory].length <= 1
        ? singleOrNull(declarationMap[key_factory])
        : null;
    ...
    return new ParsedDeclarations._(
      ...
      abstractProps: declarationMap[key_abstractProps].isNotEmpty
          ? declarationMap[key_abstractProps]
          : <ClassDeclaration>[],
      abstractState: declarationMap[key_abstractState].isNotEmpty
          ? declarationMap[key_abstractState]
          : <ClassDeclaration>[],
      propsMixins: declarationMap[key_propsMixin].isNotEmpty
          ? declarationMap[key_propsMixin]
          : <ClassDeclaration>[],
      stateMixins: declarationMap[key_stateMixin].isNotEmpty
          ? declarationMap[key_stateMixin]
          : <ClassDeclaration>[],
      hasErrors: hasErrors,
  ...
  ParsedDeclarations._({
    ...
    @Deprecated('4.0.0') ClassDeclaration component,
    ...
    bool hasAbstractStateCompanionClass,
        ...
        this.component =
            (component == null) ? null : new ComponentNode(component),
        this.component2 =
            (component2 == null) ? null : new Component2Node(component2),
        this.props = (props == null)
            ? null
            : new PropsNode(props, hasPropsCompanionClass),
        this.state = (state == null)
            ? null
            : new StateNode(state, hasStateCompanionClass),
        this.abstractProps = new List.unmodifiable(abstractProps.map((props) =>
            new AbstractPropsNode(props, hasAbstractPropsCompanionClass))),
        this.abstractState = new List.unmodifiable(abstractState.map((state) =>
            new AbstractStateNode(state, hasAbstractStateCompanionClass))),
        this.propsMixins = new List.unmodifiable(
            propsMixins.map((propsMixin) => new PropsMixinNode(propsMixin))),
        this.stateMixins = new List.unmodifiable(
            stateMixins.map((stateMixin) => new StateMixinNode(stateMixin))),
        this.declaresComponent = factory != null {
    assert(((this.factory == null &&
                ((this.component ?? this.component2) == null) &&
                this.props == null) ||
            (this.factory != null &&
                ((this.component ?? this.component2) != null) &&
                this.props != null)) &&
        '`factory`, `component` / `component2`, and `props` must be either all null or all non-null. '
                'Any other combination represents an invalid component declaration. '
            is String);
  }
  ...
  @Deprecated('4.0.0')
  static final String key_abstractComponent =
      getName(annotations.AbstractComponent);
  static final String key_abstractComponent2 =
      getName(annotations.AbstractComponent2);
  ...
  static final List<String> key_allComponentVersionsRequired =
...
class ComponentNode<TMeta extends annotations.Component>
  ...
  ComponentNode(AnnotatedNode unit) : super(unit) {
    ...
    NamedExpression subtypeOfParam =
        this.unsupportedArguments.firstWhere((expression) {
...
class FactoryNode
    extends NodeWithMeta<TopLevelVariableDeclaration, annotations.Factory> {
  FactoryNode(unit) : super(unit);
}
...
class PropsMixinNode
    extends NodeWithMeta<ClassDeclaration, annotations.PropsMixin> {
  PropsMixinNode(unit) : super(unit);
}

class StateMixinNode
    extends NodeWithMeta<ClassDeclaration, annotations.StateMixin> {
  StateMixinNode(unit) : super(unit);
}
